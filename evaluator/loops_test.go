...
func TestLoopEvaluator_RepeatedToolCallFlagged(t *testing.T) {
	...
	eval, err := (&LoopEvaluator{}).EvaluateRun(context.Background(), run)
	if err != nil {
...
func TestLoopEvaluator_DistinctInputsNotFlagged(t *testing.T) {
	...
	eval, _ := (&LoopEvaluator{}).EvaluateRun(context.Background(), run)
...
func TestLoopEvaluator_CustomThreshold(t *testing.T) {
	...
	eval, _ := (&LoopEvaluator{MaxRepeats: 2}).EvaluateRun(context.Background(), run)
	if len(eval.Findings) != 1 {
